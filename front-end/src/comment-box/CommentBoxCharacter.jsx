...
function CommentBoxCharacter({ characterId }) {
  ...
  const fetchComments = async () => {
    try {
      const response = await axios.get(
        `http://localhost:8080/comments/characters/${characterId}`
      );
  ...
  const handlePostComment = async () => {
    ...
    try {
      ...
      if (!userId) {
        message.error("You must log in to comment.");
        return;
      }
      await axios.post("http://localhost:8080/comments/characters", {
        userId,
        characterId,
        comment: newComment,
      });
  ...
  const handleSaveEdit = async () => {
    try {
      ...
      await axios.put("http://localhost:8080/comments/characters", {
        userId,
        characterId,
        commentId,
        comment: editingText,
  ...
  return (
    <div>
      {comments.length > 0 ? (
            ...
            <List.Item
              ...
              actions={[
                ...(item.users_id === userId
                  ? [
                      <Button onClick={() => handleEditComment(item)}>
                        Edit
                      </Button>,
                    ]
                  : []),
                ...(item.users_id === userId || userRole === `"admin"`
                  ? [
                      <Button
                        onClick={() =>
                          handleDeleteComment(item.Id, item.users_id)
                        }
                      >
                        Delete
                      </Button>,
        ...
        !loading && <Empty description="No comments yet" />
      )}
      <Form.Item>
        <TextArea
          rows={4}
